"""
V12 Proper Holdout Validation
Train: 2020-2024
Test: 2025 (True Out-of-Sample)
Strategy: High Conf + $3-$8 + <550m
"""
import sqlite3
import pandas as pd
import numpy as np
...
print("="*70)
print("V12 PROPER HOLDOUT VALIDATION")
print("Train: 2020-2024 | Test: 2025 (True Out-of-Sample)")
...
train_df = df[df['MeetingDate'].dt.year < 2025]
test_df = df[df['MeetingDate'].dt.year == 2025]
print(f"  Train: {len(train_df):,} entries (2020-2024)")
print(f"  Test: {len(test_df):,} entries (2025)")
...
for race_id, race_df in df.groupby('RaceID', sort=False):
    ...
    for _, r in race_df.iterrows():
        ...
        dog_history[dog_id].append({
            ...
            'position': r['Position'], 'track_id': track_id, 'distance': distance, 'tier': 0,
...
print(f"  Train features: {len(train_feat_df):,}")
print(f"  Test features: {len(test_feat_df):,}")

print("[3/5] Training model on 2020-2024...")
...
print("[4/5] Testing on 2025 (True Out-of-Sample)...")
...
print("\n[5/5] 2025 OUT-OF-SAMPLE RESULTS")
print("="*70)
...
valid = strategy_bets.dropna(subset=['BSP'])
profit_bsp = valid[valid['Won']==1]['BSP'].sum() - len(valid)
profit_bsp_comm = (valid[valid['Won']==1]['BSP'].sum() * 0.9) - len(valid)
roi_bsp = profit_bsp / len(valid) * 100
roi_bsp_comm = profit_bsp_comm / len(valid) * 100

print(f"Total 2025 Bets: {total}")
print(f"Wins: {wins}")
print(f"Win Rate: {sr:.1f}%")
print(f"\nROI (No Commission):   {roi_bsp:+.1f}%")
print(f"ROI (10% Commission):  {roi_bsp_comm:+.1f}%")
print(f"Profit ($10 stakes):   ${profit_bsp_comm * 10:+,.0f}")

# Monthly breakdown
print("\n" + "="*70)
print("MONTHLY BREAKDOWN (2025)")
print("="*70)
valid['Month'] = valid['MeetingDate'].dt.to_period('M')
for month, grp in valid.groupby('Month'):
    bets = len(grp)
    wins_m = grp['Won'].sum()
    sr_m = wins_m / bets * 100
    profit_m = (grp[grp['Won']==1]['BSP'].sum() * 0.9) - bets
    print(f"{month}: {bets} bets, {wins_m} wins ({sr_m:.1f}%), Profit: ${profit_m:+.0f}")

print("="*70)
print("VALIDATION STATUS: " + ("PASS ✓" if roi_bsp_comm > 0 else "FAIL ✗"))
print("="*70)
