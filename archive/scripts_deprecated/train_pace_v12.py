...
def train_pace_v12():
    ...
    print("\n[5/6] Training LightGBM with RandomizedSearchCV (500 iterations)...")
    ...
    random_search = RandomizedSearchCV(lgb, param_dist, n_iter=500, cv=3, scoring='roc_auc', n_jobs=1, verbose=2, random_state=42)
    random_search.fit(X_train_scaled, y_train)
    
    print(f"\nBest CV AUC: {random_search.best_score_:.4f}")
    print(f"Best params: {random_search.best_params_}")
    
    print("\n[6/6] Evaluating and backtesting...")
    ...
    print("\n--- BY TIER ---")
    for tier_name, tier_val in [("Metro", 2), ("Provincial", 1), ("Country", 0)]:
        tier_df = race_leaders[race_leaders['Tier'] == tier_val]
        backtest(tier_df[(tier_df['BSP'] >= 2) & (tier_df['BSP'] <= 10)], f"{tier_name} $2-$10")
    
    # Save
    print("\n" + "="*70)
    with open('models/pace_v12_optimized.pkl', 'wb') as f:
        pickle.dump({'model': best_model, 'scaler': scaler, 'features': feature_cols,
                     'params': random_search.best_params_, 'cv_auc': random_search.best_score_}, f)
    print("Saved to models/pace_v12_optimized.pkl")
    print("="*70)
