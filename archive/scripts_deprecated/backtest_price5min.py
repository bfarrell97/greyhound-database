"""
V12 Strategy Backtest: High Conf + $3-$8 + <550m with Price5Min
Compare results using BSP vs Price5Min
"""
import sqlite3
import pandas as pd
import numpy as np
...
print("="*70)
print("V12 STRATEGY BACKTEST: Price5Min vs BSP")
print("Strategy: High Conf (>10% margin) + $3-$8 + <550m")
print("="*70)
...
feat_df['Price5Min'] = pd.to_numeric(feat_df['Price5Min'], errors='coerce')
...
print("="*70)
print(f"Total Strategy Bets: {len(strategy_bets):,}")

wins = strategy_bets['Won'].sum()
sr = wins / len(strategy_bets) * 100

# BSP Results 
valid_bsp = strategy_bets.dropna(subset=['BSP'])
profit_bsp = valid_bsp[valid_bsp['Won']==1]['BSP'].sum() - len(valid_bsp)
profit_bsp_comm = (valid_bsp[valid_bsp['Won']==1]['BSP'].sum() * (1 - BETFAIR_COMMISSION)) - len(valid_bsp)
roi_bsp = profit_bsp / len(valid_bsp) * 100
roi_bsp_comm = profit_bsp_comm / len(valid_bsp) * 100

# Price5Min Results
valid_p5 = strategy_bets.dropna(subset=['Price5Min'])
profit_p5 = valid_p5[valid_p5['Won']==1]['Price5Min'].sum() - len(valid_p5)
profit_p5_comm = (valid_p5[valid_p5['Won']==1]['Price5Min'].sum() * (1 - BETFAIR_COMMISSION)) - len(valid_p5)
roi_p5 = profit_p5 / len(valid_p5) * 100
roi_p5_comm = profit_p5_comm / len(valid_p5) * 100

print(f"\nWin Rate: {sr:.1f}%")
print(f"\nPrice5Min Coverage: {len(valid_p5)}/{len(strategy_bets)} ({len(valid_p5)/len(strategy_bets)*100:.1f}%)")

print(f"\n{'Metric':<30} {'BSP':>15} {'Price5Min':>15}")
print("-"*60)
print(f"{'ROI (No Commission)':<30} {roi_bsp:>+14.1f}% {roi_p5:>+14.1f}%")
print(f"{'ROI (10% Commission)':<30} {roi_bsp_comm:>+14.1f}% {roi_p5_comm:>+14.1f}%")
print(f"{'Profit (Flat $10) No Comm':<30} ${profit_bsp*10:>+13,.0f} ${profit_p5*10:>+13,.0f}")
print(f"{'Profit (Flat $10) W/ Comm':<30} ${profit_bsp_comm*10:>+13,.0f} ${profit_p5_comm*10:>+13,.0f}")

diff = roi_p5_comm - roi_bsp_comm
print(f"\n-> Price5Min advantage: {diff:+.1f}% ROI points")

# Monthly breakdown
print("\n" + "="*70)
print("MONTHLY BREAKDOWN (With 10% Commission)")
print("="*70)

valid_p5['Month'] = valid_p5['MeetingDate'].dt.to_period('M')
monthly = []
for month, grp in valid_p5.groupby('Month'):
    bets = len(grp)
    wins = grp['Won'].sum()
    sr = wins / bets * 100
    p_bsp = (grp[grp['Won']==1]['BSP'].sum() * 0.9) - bets
    p_p5 = (grp[grp['Won']==1]['Price5Min'].sum() * 0.9) - bets
    monthly.append({'Month': str(month), 'Bets': bets, 'BSP_Profit': p_bsp, 'P5_Profit': p_p5})
    print(f"{month}: {bets} bets, BSP: ${p_bsp:+.0f}, Price5Min: ${p_p5:+.0f}")
