"""
Pace Model V8 - Tier-Weighted Form Features
============================================
V5 is still the best model (31.9% SR, +4.0% ROI).
V7 showed BoxType_WinRate has value.
V8 adds tier-weighted features:
1. RecentTierScore - sum of tier points (3=metro, 2=prov, 1=country)
2. MetroWinLast5 - wins at metro level in last 5
3. WeightedFormScore - wins weighted by tier
4. TierStep - stepping up/down from recent tier
5. WinRate x CurrentTier
...
def get_tier_score(track):
    """3 for metro, 2 for provincial, 1 for country"""
    if track in METRO_TRACKS: return 3
    elif track in PROVINCIAL_TRACKS: return 2
    return 1

def train_v8_model():
    print("="*70)
    print("PACE MODEL V8 - TIER-WEIGHTED FORM")
    print("="*70)
    ...
    df['TierScore'] = df['TrackName'].apply(get_tier_score)
    ...
    for race_id, race_df in df.groupby('RaceID', sort=False):
        ...
        tier_score = race_df['TierScore'].iloc[0]
            ...
            for _, r in race_df.iterrows():
                ...
                if len(hist) >= 3:
                    ...
                    if len(recent_times) >= 3:
                        ...
                        # === NEW V8 TIER-WEIGHTED FEATURES ===
                        # Recent tier scores (3=metro, 2=prov, 1=country)
                        last5 = hist[-5:] if len(hist) >= 5 else hist
                        recent_tier_score = sum(h['tier_score'] for h in last5) / len(last5)  # Avg tier quality
                        
                        # Metro wins in last 5
                        metro_wins_last5 = sum(1 for h in last5 if h['tier_score'] == 3 and h['position'] == 1)
                        metro_runs_last5 = sum(1 for h in last5 if h['tier_score'] == 3)
                        
                        # Weighted form score (wins × tier)
                        weighted_win_score = sum(h['tier_score'] for h in last5 if h['position'] == 1)
                        
                        # Tier step (compare recent avg tier to current)
                        avg_recent_tier = np.mean([h['tier_score'] for h in last5])
                        tier_step = tier_score - avg_recent_tier  # Positive = stepping up
                        
                        # Win rate × current tier
                        win_rate_x_tier = win_rate * tier_score
                        
                        # Box type win rate (from V7 - it was useful)
                        ...
                        feature_rows.append({
                            ...
                            # V7 feature that worked
                            'BoxType_WinRate': box_type_win_rate,
                            # V8 tier-weighted features
                            'RecentTierScore': recent_tier_score,
                            'MetroWinsLast5': metro_wins_last5,
                            'WeightedWinScore': weighted_win_score,
                            'TierStep': tier_step,
                            'WinRate_x_Tier': win_rate_x_tier
                        })
    ...
    feature_cols = [
        ...
        'BoxType_WinRate',
        'RecentTierScore', 'MetroWinsLast5', 'WeightedWinScore', 'TierStep', 'WinRate_x_Tier'
    ]
    ...
    with open('models/pace_v8_tier_weighted.pkl', 'wb') as f:
        pickle.dump(model_artifacts, f)
    print("Saved to models/pace_v8_tier_weighted.pkl")
    print("="*70)
