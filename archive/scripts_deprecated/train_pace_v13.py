"""
PACE V13 - Using Feature Factory V2's Top 106 Features
Quick training with 100 iterations for comparison with V12
"""
import sqlite3
import pandas as pd
import numpy as np
...
print("="*70)
print("V13 TRAINING - 106 Features from Feature Factory V2")
print("100 iterations for quick comparison with V12")
print("="*70)

# Load top features
with open('models/top_features_v2.txt', 'r') as f:
    TOP_FEATURES = [line.strip() for line in f.readlines()]
print(f"\nLoaded {len(TOP_FEATURES)} features from FF V2")

conn = sqlite3.connect('greyhound_racing.db')

print("\n[1/6] Loading benchmarks...")
...
print("[3/6] Building lookups and features...")
...
for race_id, race_df in df.groupby('RaceID', sort=False):
    ...
    # Only process 2024+ for training/test
        ...
        for _, r in race_df.iterrows():
            ...
            if len(hist) >= 3:
                ...
                if len(times) >= 3:
                    ...
                    features['RisingScore'] = features['TimeImproving'] * form_trend * -1
...
print(f"  Total: {processed:,} races, {len(feature_rows):,} feature rows")
...
# Use only FF V2 top features that exist
available_features = [f for f in TOP_FEATURES if f in feat_df.columns]
print(f"  Using {len(available_features)} of {len(TOP_FEATURES)} features")
...
X_train = train_df[available_features].fillna(0).replace([np.inf, -np.inf], 0)
y_train = train_df['Won']
X_test = test_df[available_features].fillna(0).replace([np.inf, -np.inf], 0)
y_test = test_df['Won']

print("[5/6] Training V13 (100 iterations, GPU)...")
...
# 100 iterations for quick comparison
random_search = RandomizedSearchCV(lgb, param_dist, n_iter=100, cv=3, scoring='roc_auc', 
...
with open('models/pace_v13_optimized.pkl', 'wb') as f:
    pickle.dump(model_data, f)
print("Saved to models/pace_v13_optimized.pkl")
...
print("="*70)
print("COMPARISON: V12 vs V13")
print("="*70)
print("V12: CV AUC 0.6933, 69 features, $3-$8 ROI: +5.5%")
print(f"V13: CV AUC {random_search.best_score_:.4f}, {len(available_features)} features")
