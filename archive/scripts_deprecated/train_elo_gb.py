"""
Enhanced Elo Model with GradientBoosting
=========================================
Uses Elo ratings as features, trains GradientBoostingClassifier to predict wins.
Features: Elo rating, gap to 2nd, rank in race, track tier, etc.
...
def train_elo_model():
    print("="*70)
    print("ENHANCED ELO MODEL WITH GRADIENT BOOSTING")
    print("="*70)
    ...
    # [2/6] Calculate Elo ratings and build features
    ...
    for race_id, race_df in df.groupby('RaceID', sort=False):
        ...
        for _, r in race_df.iterrows():
            ...
            race_elo[dog_id] = elo_ratings[dog_id]
        ...
        for idx, (_, r) in enumerate(race_df.iterrows()):
            ...
            # Only create training features for races in training period
            # And test features for test period
            feature_rows.append({
                'RaceID': race_id,
                'GreyhoundID': dog_id,
                'Won': r['Won'],
        ...
        for _, r in race_df.iterrows():
            ...
            # Margin bonus for wins
            margin_bonus = 0
            if actual == 1.0 and r['Margin'] > 0:
                margin_bonus = min(r['Margin'] / 10, 0.5)  # Up to 50% K bonus for big wins
    ...
    # [4/6] Train with GridSearchCV
    print("\n[4/6] Training GradientBoostingClassifier with GridSearchCV...")
    ...
    # Find highest probability per race
    ...
    # Backtest at different odds ranges
    print("\n" + "="*70)
    ...
    with open('models/elo_gb_model.pkl', 'wb') as f:
        pickle.dump(model_artifacts, f)
    print("Saved model to models/elo_gb_model.pkl")
    print("="*70)
