# explore_v30_2025_earliest.py
"""
Evaluate the V30 AutoGluon model (BSP features) on the entire year 2025 using the earliest
available price column for each runner (Price2Hr, Price60Min, …, Price1Min).
If none of those columns have a value, fall back to the BSP (StartPrice).
The script prints price‑accuracy metrics (MAPE, correlation) and a betting
simulation with a value threshold of 50% and a maximum price of $10 (10% commission).
"""
import sqlite3
import pandas as pd
import numpy as np
...
df['BSP_log'] = np.log10(df['StartPrice'])
...
for col in price_order:
    if col not in df.columns:
        df[col] = np.nan
# Back‑fill across the ordered columns to get the earliest available price
...
# ---------- 4. ROLLING WINDOW FEATURES (same as V30) ----------
...
print(f"Model dataframe rows: {len(model_df):,}, feature columns: {len(feature_cols)}")
print("Sample feature columns:", list(feature_cols)[:10])

# ---------- 5. LOAD V30 MODEL ----------
predictor = TabularPredictor.load('models/autogluon_v30_bsp')
model_features = set(predictor.feature_metadata.get_features())
missing = model_features - set(feature_cols)
if missing:
    print(f"Warning: {len(missing)} model features missing in data. They will be filled with 0.")
    for col in missing:
        model_df[col] = 0
...
# Price accuracy using earliest price
...
print("=== V30 2025 (Earliest Price) Evaluation ===")
print(f"Races: {races:,}")
