"""Test: Gap >= 0.15, Middle Dist (400-550m), SP $3-$8"""
import sqlite3
import pandas as pd
import pickle
...
SELECT ge.GreyhoundID, r.RaceID, rm.MeetingDate, t.TrackName, r.Distance, ge.Box,
       ge.FinishTime, ge.Position, ge.StartingPrice
...
# Parse SP (often stored as string like "$3.50")
...
# CONFIG: Gap >= 0.15, Middle Dist (400-550m), SP 3-8
strat = leaders[
    (leaders['Gap'] >= 0.15) &
...
strat['Profit'] = strat.apply(lambda x: (x['SP'] - 1) if x['Position'] == '1' else -1, axis=1)
strat['Year'] = strat['MeetingDate'].dt.year

print('='*70)
print('STRATEGY: Gap >= 0.15, Middle Dist (400-550m), SP $3-$8')
print('='*70)
