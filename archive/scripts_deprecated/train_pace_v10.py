"""
Pace Model V10 - Proven Features Only
=====================================
Built on the best performing model (V5, +4.0% ROI).
Adds ONLY the single feature that proved useful in later iterations:
1. BoxType_WinRate (+4% importance in V7/V9)
...
- Beyer Speed Figures
- Age & Experience
- Interaction terms (TimeBest_x_Trainer, etc.)

Removed Failed Initialiments:
- RecentAvgBSP (Market Leakage)
- Roll3/Roll5 (Hurt ROI)
- Tier-weighted stats (Minimal impact)
- Prize Money (Minimal impact)

Goal: Refine the best model for maximum stability and ROI.
"""
import sqlite3
import pandas as pd
import numpy as np
...
def train_v10_model():
    print("="*70)
    print("PACE MODEL V10 - PROVEN FEATURES ONLY")
    print("="*70)
            ...
            for _, r in race_df.iterrows():
                ...
                if len(hist) >= 3:
                    ...
                    if len(recent_times) >= 3:
                        ...
                        # === V10 ADDITION: Box Type Win Rate ===
    ...
    grid_search = GridSearchCV(lgb, param_grid, cv=3, scoring='roc_auc', n_jobs=1, verbose=2)
    ...
    with open('models/pace_v10_proven.pkl', 'wb') as f:
        pickle.dump(model_artifacts, f)
    print("Saved to models/pace_v10_proven.pkl")
    print("="*70)
