...
def feature_engineering(df):
    ...
    # Calculate Race Overround
    # Group by RaceID (or Meeting/RaceNum) to sum probs
    # Note: RaceID is unique across DB usually? Let's assume yes or use RaceKey
    df['RaceKey'] = df['MeetingDate'].astype(str) + '_' + df['TrackName'] + '_' + df['RaceID'].astype(str) # Robust key
    
    # Sum of implied probs per race
    df['RaceOverround'] = df.groupby('RaceKey')['ImpliedProb'].transform('sum')
    
    # Normalize to get "True" Market Prob (removes margin)
    df['MarketTrueProb'] = df['ImpliedProb'] / df['RaceOverround']
...
def evaluate_strategy(df):
    print("\n" + "="*80)
    print("VALUE BETTING STRATEGY (Model vs TRUE Market Prob) (2025)")
    print("="*80)
    
    # Strategy: Bet if ModelProb > MarketTrueProb * Threshold
    # Using MarketTrueProb fairer comparison (apples to apples sum=1)
    ...
    for thresh in thresholds:
        # Edge calculation using True Prob
        mask = (df['ModelProb'] > df['MarketTrueProb'] * thresh) & (df['Odds'] >= 1.50) & (df['Odds'] <= 30)
        bets = df[mask].copy()
