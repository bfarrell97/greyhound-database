...
def run_backtest():
    ...
    lay_stats = {
        ...
        'Hedged_Big_Steam': {'bets': 0, 'pnl': 0.0, 'invested': 0.0, 'details': []},
    ...
    for idx, row in df_bt.iterrows():
        ...
        # BSP COMPARISON & HEDGING LOGIC
        bsp = row['BSP'] if 'BSP' in row and row['BSP'] > 0 else 1.01
        beat_bsp = False
        
        # Original Status for tracking (Did we beat market?)
        if signal == 'LAY':
             if exec_price < bsp: beat_bsp = True
        elif signal == 'BACK':
             if exec_price > bsp: beat_bsp = True
             
        # HEDGING SIMULATION (User Query v2)
        # Only hedge "Massive Steamers" (> 20% Price Drop)
        result_type = "Standard"
        
        if signal == 'LAY' and exec_price > bsp:
            # Calculate Drop Percentage
            # e.g. Lay 10, BSP 8. Drop 2. Pct = 2/10 = 20%
            drop_pct = (exec_price - bsp) / exec_price
            
            if drop_pct >= 0.20:
                # MASSIVE STEAMER - STOP LOSS
                pnl = stake * (1.0 - (exec_price / bsp)) # Guaranteed PnL (Loss)
                result_type = "HEDGED_BIG_STEAM"
            else:
                # Moderate Steam - Let it ride
                pass
        ...
        # Track Lays specifically
        if signal == 'LAY':
            is_missed = (exec_price > bsp)
            stats_key = 'Missed_BSP' if is_missed else 'Beat_BSP'
            
            # Additional bucket for Big Steamers
            if result_type == "HEDGED_BIG_STEAM":
                 stats_key = 'Hedged_Big_Steam' # Create this key in dict logic below or just track in details?
                 # Let's just track in Missed_BSP for now but note the type
        ...
        # Prevent bankruptcy
        # if curr_bank <= 5.0: # Disable for hedging test to see full result
        #     print("❌ BANKRUPTCY!")
        #     break
    ...
    print("\n" + "="*60)
    print("LAY STRATEGY: HEDGING SIMULATION")
    print("Strategy: If Lay Price > BSP, Hedge (Red Up) at BSP.")
    print("="*60)
    
    # 1. Missed BSP (Price > BSP) -> Higher Liability than SP
    # Split into HEDGED (Big Steam) and UNHEDGED (Moderate Steam)
    
    # 1a. Moderate Steam (Let Ride)
    ...
    print(f"\n[MISSED BSP - MODERATE] (< 20% Drop) -> Let Ride")
    print(f"  Bets: {m_bets}")
    print(f"  P&L:  ${m_pnl:.2f}")
    print(f"  ROI:  {m_roi:.2f}%")

    # 1b. Big Steam (Hedged)
    h_bets = lay_stats['Hedged_Big_Steam']['bets']
    h_pnl = lay_stats['Hedged_Big_Steam']['pnl']
    h_inv = lay_stats['Hedged_Big_Steam']['invested']
    h_roi = (h_pnl / h_inv * 100) if h_inv > 0 else 0.0
    
    print(f"\n[MISSED BSP - MASSIVE] (>= 20% Drop) -> **HEDGED**")
    print(f"  Bets: {h_bets}")
    print(f"  P&L:  ${h_pnl:.2f} (Locked Loss)")
    print(f"  ROI:  {h_roi:.2f}% (vs Original Risk)")
    
    # 2. Beat BSP (Price <= BSP) -> UNHEDGED
    ...
    print(f"\n[BEAT BSP] (Lay Price <= BSP) -> Let Ride")
    print(f"  Bets: {b_bets}")
    print(f"  P&L:  ${b_pnl:.2f}")
    print(f"  ROI:  {b_roi:.2f}%")
    
    # Combined
    tot_bets = m_bets + b_bets + h_bets
    tot_pnl = m_pnl + b_pnl + h_pnl
    tot_inv = m_inv + b_inv + h_inv
